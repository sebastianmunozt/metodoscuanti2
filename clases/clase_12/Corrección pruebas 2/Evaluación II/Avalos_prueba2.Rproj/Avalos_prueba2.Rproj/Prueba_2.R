...
#El valor p en estadistica indica provabilidad, el cual para ser utilizado se necesitan criterios, por ejemplo cuando el 0,05 significa que es predefinido y se rechaza la hipotesis nula. 
#Es importante destacar que el valor-p es una medida de la probabilidad de obtener un resultado igual o más extremo que el observado, asumiendo que la hipótesis nula es cierta.

### Falta claridad y precisión en la respuesta, pues no se entiende a qué se refiere con que el valor sea predefinido. 
### Es importante mencionar que el valor -p se obtiene a partir del nivel de confianza, por lo que su interpretación dependerá de esto.
...
#   La estadística descriptiva es el conjunto de métodos estadísticos que describen y/o caracterizan un grupo de datos, mientras que la estadística inferencial busca deducir y sacar conclusión
#La estadística descriptiva se encarga de procesar y organizar los datos, mostrando resúmenes numéricos o gráficos que permiten comprender las características fundamentales de las variables. 
#Por otro lado, la estadística inferencial utiliza una muestra aleatoria de datos de una población para hacer inferencias y sacar conclusión

### Está bien :) Creo que es importante mencionar también que la estadística inferencial pretende extrapolar los resultados de los datos desde una muestra a una población,
### es decir, extraer conclusiones a partir de una muestra, para referirnos a la población.

# 5. ¿cuál es la diferencia entre las pruebas de contraste de hipótesis de Chi cuadrado con ANOVA?
#   se utiliza Chi cuadrado para evaluar existencia de asociacion y relacion de variables categoricas, si es estadisticamente significativo desde tablas de contingencia y mediante estas se testean los analisis estadisticos.

### Falta contemplar la respuesta. No se refiere al test de ANOVA y en qué aspectos se diferencian ambos contrastes
...
libro_codigo <- read.xlsx("base_antropologia_limpia.xlsx")
...
str(base)
summary(base)
view(base) #para ver la base se utiliza con mayúsculas en la V.
nrow(base) #147 cantidad de casos
ncol(base) #60 cantidad de variables
sapply(base, FUN = class)
str(base) #se repite 
...
#                                ______ = ______, 
#                                ______ = ______)
base <- base %>% dplyr::rename(tiempo_libre_Semana = to_01)#TO_01. ¿Cuánto tiempo libre considera que ha tenido por semana para dedicarse a actividades fuera del ámbito universitario? (Considere actividades fuera del ámbito universitario tales como deporte, entretenimiento, salidas, fiestas, etc)"
base <- base %>% dplyr::rename(disposicion_tiempo_ocio = to_02)# "TO_02 Considere esta definición de ocio antes de contestar ésta y las siguientes preguntas: “El ocio se puede entender como el tiempo del que dispone una persona para distraerse e idealmente disfrutar de un momento agradable en su agenda personal o tiempo libre” Tales como salir tener vida social, hacer deporte o prácticas de consumo o participación cultural (leer, escuchar o tocar música, estar en internet) Excluya trabajo y estudio Ahora que ya comprende a qué se refiere el ocio en esta encuesta, responda: ¿Cuántos días a la semana (incluyendo semana y fin semana) realiza actividades de ocio?"
base <- base %>% dplyr::rename(administracion_tiempo_optima = to_03) # "TO_03 ¿Administra su tiempo de forma óptima para equilibrar tus obligaciones académicas con las actividades de ocio?"
base <- base %>% dplyr::rename(actividad_tiempo_ocio = to_04) #TO_04. ¿Cuáles de estas actividades prefiere realizar en su tiempo de ocio? 
names(base)   

#si bien no se aplica correvtamente el código, igualmente se logra el objetivo, recodificando una por una las variables. 
...
#En su casa o de familiares, En casa de amigos, En el transporte público, En el lugar de estudio, En otros lugares. 
...
unique(base&lugar_musica) #sale error poraue se utilizó & en vez de $
...
respuestas <- gsub("[·]", "", respuestas) #esto sirve para eliminar los puntos, no se solicitaba en esta parte
...
#Aquí ocurrió que no se te repitieron las categorías de respuesta, porque copiaste el codigo y lo pegaste arriba en el ítem 03.4, 
#por lo que no se da lo esperado para desarrollar el ítem 03.6
#se evidencia que no hay un entendimiento total de lo que se está aplicando
...
victor_avalos <- freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
  tb()
...
Espiritualidad sin afiliación religiosa
Religión específica
Sin religión
...
religion_rec <- base %>%  mutate(religion_rec= re_02(case_when(re_02 %in% c("No tengo una afiliación religiosa, pero si me considero una persona espiritual (por ejemplo, creo en las energías)","seria en creer en alguien superior un tipo de energía que se interpreta de diversas formas en la religión pero no es alguien al cual se le pueda poner un nombre", "deísmo") ~ "Espiritualidad sin afiliación religiosa",
...
base$ea_04_notas_ultimo_semestre <- as.numeric(base$ea_04_notas_ultimo_semestre)
base$ea_04_notas_ultimo_semestre <- as.numeric(base$ea_04_notas_ultimo_semestre)

# Media (0.5 ptos)
mean_notas <- mean(base$ea_04_notas_ultimo_semestre, na.rm = TRUE)

mean(base$ea_04_notas_ultimo_semestre, na.rm = TRUE) #La media es de 5.69
# Mediana (0.5 ptos)

median(base$ea_04_notas_ultimo_semestre, na.rm = TRUE) #La mediana es de 5.8

# desviación estándar (0.5 ptos)

sd(base$ea_04_notas_ultimo_semestre, na.rm = TRUE) #La desviación es de 0.47

# varianza (0.5 ptos)

var(base$ea_04_notas_ultimo_semestre, na.rm = TRUE) #La varianza es de 0.22
...
ggplot(base, aes(x, y=____, fill=____)) + 
...
ggplot(victor_avalos, aes(x = pct, y = fct_reorder(respuestas, pct), fill=respuestas)) +
  geom_col() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  xlab("") + ylab("") +
  labs(title = "Lugares donde escuchan música", 

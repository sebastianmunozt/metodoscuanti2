...
### FALTÓ RESPONDER LAS PREGUNTAS DE DESARROLLO :(
...
base <- base %>% dplyr::rename(tiempo_libre_semanal= to_01,#¿Cuánto tiempo libre considera que ha tenido por semana para dedicarse a actividades fuera del ámbito universitario?
                               diasxsemana_actividad_ocio= to_02,#¿Cuántos días a la semana (incluyendo semana y fin de semana) realiza actividades de ocio?
                               ...
                               actividades_ocio=to_04)#¿Cuáles de estas actividades prefiere realizar en su tiempo de ocio?
...
#Está bien la función utilizada, pero hizo falta poner la categoría exactamente como te salía para poder cambiarla, es decir, si sale con punto, debes ponerla de esa misma forma
#En este caso solo repetiste las categorías y las pusiste de la misma manera en los dos lados, siendo que primero debías poner la categoría inicial, y luego la categoría como querías que quedara.
...
sofia_ballerino <-  freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
  tb()
...
base <- base %>% dplyr::rename(religion_rec= re_02) ### Que horrible, este paso no iba. Entendí mal la instrucción, pero abajo dejé el código igual c:
base$religion_rec
...
ggplot(base, aes(x= religion_rec, y= ea_04_notas_ultimo_semestre , fill= ea_04_notas_ultimo_semestre )) + 
  geom_boxplot(alpha=0.3) +
  labs(title = "Promedio de notas según tipo de religión", 
       caption = "Fuente: Encuesta de Estudiantes UAH 2024",
       y= "Notas último semestre",
       x= "Religión",
       fill = "Religión")+
...
ggplot(base, aes(x=tiempo_libre_semanal, y= ea_04_notas_ultimo_semestre, fill= ea_04_notas_ultimo_semestre)) + 
  geom_boxplot(alpha=0.3) +
  labs(title = "Notas último semestre según tiempo", 
       caption = "Fuente: Encuesta de Estudiantes UAH 2024",
       y= "Notas último semestre",
       x="Tiempo libre por semana",
       fill = "Tiempo libre semanal")+
  theme(legend.position="none")
...
ggplot(sofia_ballerino, aes(x = pct, y = fct_reorder(respuestas, pct), fill=respuestas)) +
  geom_col() +

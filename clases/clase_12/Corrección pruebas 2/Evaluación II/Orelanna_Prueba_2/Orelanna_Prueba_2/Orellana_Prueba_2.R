...
# 1. Describa la diferencia entre la hipótesis nula y la hipótesis alternativa en un contraste de hipótesis.
    ...
    #R: La principal diferencia entre la hipotesis nula y la alrernativa es que son mutuamente opuestas
    #   La hipotesis alternativa es lo que ekk investigador busca comprobar, mientras la nula es un puto
    #   referencial el cual se busca refutar mediante la nueva hipotesis.
    #   Un ejemplo es el problema de los sisnes de cuello negro y cuello blanco, en el cual se plantea
    #   que en un pueblo solo hay sisnes de cuello negro, por lo tanto solo existe esta variedad (hipotesis nula)
    #   pero se plantea la posibilidad de qque fuera del pueblo exista otro tipo de cisnes de cuello blanco (hipotesis alternativa)

### Lamentablemente la respuesta no es correcta, pues no te estás basando en hipótesis estadísticas, sino en hipótesis simplemente. 
### Las hipótesis nula y alternativa sirven para evaluar si existe relación o no entre las variables. 
...
# 2. ¿cuál es la manera en que se debe interpretar el valor-p y cómo repercute este valor sobre la decisión acerca del contraste de hipótesis?
    ...
    #R: El valor-p sirve para visualizar el marjen de error que puede tener los resultados optenidos en una muestra 
    #  en comparacion con el total de la poblacion. 
    #  En ciencias sociales se trabaja con 95% de certeza, por lo tanto este valor tiene que ser inferior a 0,05,
    #  de lo contrario la hipotesis que se busca comprobar queda descartada al tener un margen de rror que supera
    #  aceptable, por lo que se mantiene la hipotesis nula.

### Falta precisión y claridad en la respuesta, pues con el valor -p no hablamos de margen de erro, sino que hablamos de probabilidad.
### Es importante mencionar que el valor -p se obtiene a partir del nivel de confianza (tal como mencionas), por lo que su interpretación dependerá de esto.
...
### Con un 95% de confianza, si el valor -p no supera un valor de 0,05, es decir, si el valor -p es menor a 0,05, 
### se rechaza la hipótesis nula, pues no se alcanza la probabilidad, es decir, esta no sería probable
...
### No se responde a la pregunta de cuántas filas y columnas tienen los datos,
### por lo que no se logra evidenciar si se entiende la diferencia entre casos y variables
### Igualmente los códigos son los correctos
...
base <- base %>% dplyr::rename(horas_estudio_semana = ea_01_horas_estudio_semana,
                              horas_estudio_fin_semana = ea_02_horas_estudio_fin_semana,
                              descripcion_carga_academica = ea_03_descripcion_carga_academica, 
                              notas_ultimo_semestre = ea_04_notas_ultimo_semestre)

### Error in `dplyr::rename()`:
# ! Names must be unique.
# ✖ These names are duplicated:
#  * "horas_estudio_semana" at locations 13 and 56.
# Run `rlang::last_trace()` to see where the error occurred.

### Sale error al hacer la recodificación, por lo que no logra realizarse correctamente.
### Igualmente, el formato era el correcto, por lo que se otorgará la mitad del puntaje.
...
?gsub
 ...
 respuestas <- gsub(" En el transporte público", "En el transporte público", "^\\s+", "") #(1 pto)

 respuestas <- ____  #(1 pto)
...
mean(base$ea_04_notas_ultimo_semestre, na.rm = TRUE)
...
sd(base$ea_04_notas_ultimo_semestre, na.rm = TRUE)

# varianza (0.5 ptos)

var(base$ea_04_notas_ultimo_semestre, na.rm = TRUE)

...
# a. Ir a File y crear un Rproject denominelo: apellido_prueba2.Rproj, por ejemplo: "Ocampo_prueba2.Rproj" (#listo)
...
#   #La estadistica descriptiva tal como su nombre infiera busca describir y presentar en una organizacion
    #de datos para examinar y visualizar de mejor manera las tendencias o distribuciones de una base de datos. 
    #Mientras que la estadistica inferencial se basa en la descripción de una muestra para sacar 
    #conclusiones sobre la población de la que proviene. Generando una generalizacion sobre la informacion de 
    #la muestra, cosa que la estadistica descriptiva no hace. 

### Muy bien!

# 5. ¿cuál es la diferencia entre las pruebas de contraste de hipótesis de Chi cuadrado con ANOVA?
    #La diferencia entre las pruebas de contraste de hipotesis entre Chi cuadrado y ANOVA. 
    #Es que primeramente Chi cuadrado sirve para determinar si existe una diferencia significativa 
    #entre los resultados esperados y los observados en una o más categorías, nominales u ordinales.
    #Mientras que ANOVA es una tecnica estadistica que ayuda a comparar las medias de dos o más grupos 
    #(muestras) para determinar si existen diferencias estadísticamente significativas entre ellas.
...
# 8. ¿Por qué se dice que el proceso de inferencia sigue una lógica inductiva? 
   #Porque el proceso de inferencia es un analisis del cual se derivan conclusiones a partir
   #de premisas o hipótesis iniciales. Puesto que de una conclusión se sugieren premisas o hipótesis
   #de partida, que por medio de deducciones lógicas válidas, se implica (infiere) la conclusión.
...
nrow(base) #Número de fila 147
ncol(base) #Número de columnas 60.  
...
# Puede utilizar el siguiente formato # base <- base %>% ______::_____(Nombre nuevo = nombre antiguo,
names(base)
base <- base %>% dplyr::rename(Fuera_del_ambito_academico = to_01,  
                               Administrar_tiempo = to_03,
                                Actividades_ocio = to_04, 
                                Importancia_tiempo = to_06)
...
Daniela_Pasmiño<- freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
  tb()
...
#No me genero las nuevas categorias, desconozco el motivo. 

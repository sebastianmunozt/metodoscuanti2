...
# La diferencia entre hipótesis nula y la hipótesis alternativa es que, la primera consiste en un enunciado relativo al valor de un parámetro poblacional que se formula con el fin de probar evidencia numérica. Por otro lado, la hipótesis alternativa es un enunciado que se acepta si los datos de la muestra ofrecen suficiente evidencia para derogar la hipótesis nula.  

### Falta precisión en la respuesta
### Ambas hipótesis son afirmaciones que buscan probar si las variables o grupos evaluados se relacionan o no, 
### No es correcto que la hipótesus nula corresponde al parámetro poblacional que se formula con el fin de probar evidencia numérica.
### Las hipótesis evaluan si existe relación o no entre las variables, a través de test de hipótesis (Chi cuadrado y ANOVA) 
...
# La diferencia entre estadística descriptiva y estadística inferencial es que la primera emplea técnicas y procedimientos que permiten organizar, describir, presentar y analizar datos,en tanto, la estadística inferencial analiza información a partir de una muestra extraída de la población para reliazar inferencias dada su naturaleza probabilística. 

### En general está bien

# 5. ¿cuál es la diferencia entre las pruebas de contraste de hipótesis de Chi cuadrado con ANOVA?
#   La diferencia entre las pruebas de contraste de hipótesis de Chi cuadrado con Anova es que la primera, evalúa la independencia entre dos variables categóricas, comparando las frecuencias observadas con las frecuencias esperadas bajo la hipótesis nula de independencia. En cambio, Anova compara las medias de tres o más grupos para identificar variables significativas. 

### En general está bien, pero me parece raro que en la pregunta 1, sobre las hipótesis, dijeras algo totalmente distinto a la respuesta de esta pregunta. 
### En esta respuesta se logra identificar en qué consisten los test de hipótesis y, por ende, se asume que se identifican las hipótesis con claridad, 
### no obstante arriba respondiste algo totalmente erróneo, que no se condice con esta respuesta. Ojo con el uso de IA. 
...
nrow(base) # 147 cantidad de casos/filas
ncol(base) # 60 cantidad de variables/columnas
...
base <- base %>% dplyr::rename(tiempo_libre_semanal= to_01,
                               actividades_ocio= to_04,
                               participacion_colectivos= to_05,
                               importancia_ocio= to_06)
...
# 03.6 homologue ambas categorías de respuesta eliminando el espacio que antecede utilizando la función gsub
 ...
 ### No se realiza correctamente este paso.
 ### Te faltó poner el espacio en las primeras categorías, para reemplazar esas
 ### Tal como está en el ejemplo que les pusimos.
...
# 03.7 vuelva a observar sus respuestas con freq (1 pto)
 ____(____, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
  ...
  ### Aquí podemos evidenciar que no funcionó la eliminación de respuestas duplicadas.
...
# el nombre del objeto debe ser su "nombre_apellido", (0.5 pto)
  ...
  olivier_delherbe <-  freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
   tb()
...
table(base$tiempo_libre_semanal)
...
base <- base %>% mutate(tiempo_libre_semanal=case_when(tiempo_libre_semanal %in% c("Bastante tiempo.", "Suficiente tiempo.")~"Tiempo disponible",
                                                       tiempo_libre_semanal %in% c("No tengo tiempo.", "Poco tiempo.")~ "Tiempo limitado",
                                                       TRUE~tiempo_libre_semanal))


# 06. Obtenga los siguientes estadísticos descriptivos de la variable "ea_04_notas_ultimo_semestre"

base$ea_04_notas_ultimo_semestre
...
sd(base$ea_04_notas_ultimo_semestre, na.rm = TRUE) #La desviación estándar es 0.47

# varianza (0.5 ptos)

var(base$ea_04_notas_ultimo_semestre, na.rm = TRUE) #La varianza es 0.22

...
# Una hipótesis nula es la afirmación inicial que se supone que es verdadera, representando la idea que no hay efecto o relación entre las variables
# de estudio.
# La hipótesis alternativa es la afirmación que se busca probar, representando que existe efecto, relación o diferencia significativa entre las variables
# de estudio. No son resutados al azar y en un contraste de hipótesis se busca evidencia en la hipóteisis alternativa en lugar de la hipótesis nula. 
...
# Las medidas de tendencia central son un conjunto de herramientas / cálculos estadíticos que se utilizan para encontrar 
# un valor representativo de la muestra de datos estudiados. Por ejemplo, la media o promedio, otro ejemplo podría ser la
# moda, siendo ésta, el dato que se repite más veces. 
# La diferencia que existe entre las medidas de dispersión y las medidas de tendencia central, es que las medidas de dispersión 
# buscan identificar los grupos que se diferencian mucho entre sí, mientras que los de tendencia central, buscan identificar los
# datos más repetidos y aglomerados. 

### En general está bien, sin embargo, la representatividad de la muestra va de la mano de otros factores, como por ejemplo el muestreo. 
### El azar o la aleatoriedad en el muestreo permiten que una muestra sea representativa, lo cual no depende de las medidas de tendencia central. 
### Por otro lado, las medidas de dispersión pretenden identificar cuán dispersos están los datos de la muestra, es decir, buscan identificar la dispersión y distribución de los datos.
### Mientras que las medidas de tendencia central pretenden identificar tendencias, como por ej los datos que más se repiten
...
### Te faltó responder una pregunta :(
...
nrow(base)
ncol(base)
# La base tiene 147 filas y 60 columnas. 
...
base <- janitor::clean_names(base) #esto no era necesario

base <- base %>% dplyr::rename(tiempo_libre_act = to_01,
                               frec_tiempo_ocio = to_02,
                               organiz_tiempo_ocio = to_03,
                               importancia_tiempo_ocio = to_06)
...
respuestas <- gsub(" En el transporte público", "En el transporte público",  _____ ) #(1 pto)
...
to_01 <- base %>% mutate(tiempo_libre_act = case_when(to_01 %in% c("Bastante tiempo.", "Suficiente tiempo.")~"Tiempo disponible",
                                        tiempo_libre_act %in% c("No tengo tiempo.", "Poco tiempo.")~ "Tiempo limitado",
                                        TRUE~tiempo_libre_act))

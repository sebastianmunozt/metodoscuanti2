...
# 1. Describa la diferencia entre la hipótesis nula y la hipótesis alternativa en un contraste de hipótesis.
   #La hipótesis es algo que un investigador intenta confirmar o negar, la hipótesis nula es la que el investigador intenta rechazar, mientras que la hipótesis alternativa es aquella 
   #que el investigador intenta confirmar, si lo consideramos con variables la hipótesis nula dice que estas no tienen relacion entre si, mientras que la alternativa dice que si tienen relación entre si.
...
# 6. ¿a qué se refiere con que la correlación no establece una dependencia entre variables?
   #Se refiere a que las variables no se encuentran relacionadas entre sí, que una no afecta a la otra y viceversa, se puede decir que las variables corresponden a una hipótesis nula.

### Se presenta una confusión
...
#Por que se encuentra basado en generalizaciones hechas a mediante la observacion de ciertos casos
...
base <- base %>% dplyr::rename(tiempo_libre_semana = to_01,
                               dia_semana_ocio = to_02,
                               administracion_tiempo = to_03,
                               importancia_t_ocio = to_06)
names(base)
...
# 03.2 observe las categorías de la variable lugar_musica, puede utilizar funciones como 
#freq, table, unique, etc. (0.5 ptos)
...
# el nombre del objeto debe ser su "nombre_apellido", (0.5 pto)
 alejandra_mondaca <- freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
   tb()
...
tiempo_libre <- base %>% mutate(tiempo_libre_semana=case_when(tiempo_libre_semana %in% c("Bastante tiempo.", "Suficiente tiempo.")~"Tiempo disponible",
                                                              tiempo_libre_semana %in% c("No tengo tiempo.", "Poco tiempo.")~ "Tiempo limitado",
                                        TRUE~ tiempo_libre_semana))

# 06. Obtenga los siguientes estadísticos descriptivos de la variable "ea_04_notas_ultimo_semestre"

# Media (0.5 ptos) #5.691379
mean(base$ea_04_notas_ultimo_semestre, na.rm = TRUE)
# Mediana (0.5 ptos) #5.8
median(base$ea_04_notas_ultimo_semestre, na.rm = TRUE)
# desviación estándar (0.5 ptos) #0.4721033
sd(base$ea_04_notas_ultimo_semestre, na.rm = TRUE)
# varianza (0.5 ptos) #0.2228816
...
ggplot(base, aes(x=re_02, y=ea_04_notas_ultimo_semestre, fill=ea_04_notas_ultimo_semestre)) + 
...
ggplot(base, aes(x=tiempo_libre_semana, y=ea_04_notas_ultimo_semestre, fill=ea_04_notas_ultimo_semestre)) + 
  geom_boxplot(alpha=0.3) +
...
ggplot(alejandra_mondaca, aes(x = pct, y = fct_reorder(respuestas, pct), fill=respuestas)) +
  geom_col() +

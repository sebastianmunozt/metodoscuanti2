...
#pregunta 1: mientras que la hipotesis nula actua como una posibilidad de la cual no se puede negar, sin embargo puede cambiar o terminar por
#descartarse, la hipotesis alternativa es irrovocable, y por tanto nunca cambia; no existe relacion entre las variables, por tanto 
#se hacen contraria una a la otra esto ocurre debido que la metdologia cuantitavia es deductiva por tanto
#actua inductivamente al ejecutarse estos tipos de hipotesis. 

### La respuesta proporcionada da luces de una diferencia entre ambas hipótesis, mencionando que una es contraria a la otra, 
### sin embargo, presenta bastantes errores de coherencia :( 
### En primer lugar, se presenta una incoherencia cuando se habla de que la metodología cuantitativa es deductiva y por tanto actúa inductivamente.
### La lógica deductiva es diferente a la lógica inductiva, 
### Por otro lado, se menciona que la hipótesis nula actua como una posibilidad de la cual no se puede negar, sin embargo puede terminar por descartarse,
### Lo cual también es incoherente y a su vez es falso, puesto que la hipótesis nula sí puede ser negada si se confirma la hipótesis alternativa. 
...
#pregunta 5 : mientras que chi cuadrado se encarga de velar por la relación entre dos variables categoricas, en la cual se trabaja, en las 
#frecuencias dentro de las tablas de contigencia, por lo que nos ayuda analizar datos categoricos a su vez estas relaciones conformadas. mientras que 
#ANOVA tiene como proposito realizar comparativas de las medias en los grupos, comprendiendo a si las diferencias que se recalcan en grupos estadisticos concretos.
#esta diferencia se enuncia dependiendo de los datos que quisieramos evaluar.

### En general está bien :) 

#pregunta 6: primeramente esto se afirma por el hecho de que no se depende de una CAUSALIDAD, ya que las correlaciones
#se definen y trabajan fuerza y linealidad en relacion a dos variables expuestas, variables contienen diferentes categorias 
#en donde una de ellas es variable independiente y variable dependiente, por tantos los efectos on diferentes entre estas.
#la información que nos puede entregar las correlaciones son limitadas, no indican el origen de la formación de estas relaciones
#de manera causal ni tampoco su direccion; la interpretación de las variables se pueden ver mal interpretadas.
...
summary(base)
names(base)
view(base) #Este se utiliza con mayúscula al inicio

#La respuesta está bien, aunque podrían haberse utilizado menos funciones para obtener esta información, como nrow y ncol :)
...
(no se ve la cuarta variable, porque ya se habia realizado de to_04 a equilibrio_ocio, me habia enredado un poco 
  en los ordenes)

### En general vas bien :) hiciste bien la recodificación, pero faltó agregar una 4ta variable:c podía ser to_05, to_06 en el caso de no poder con to_04
...
base$lugar_musica <- strsplit(base$musica, ",") ###este es erróneo, pero el primero sí resulta 
...
### Súper hasta el momento :) Felicitaciones!

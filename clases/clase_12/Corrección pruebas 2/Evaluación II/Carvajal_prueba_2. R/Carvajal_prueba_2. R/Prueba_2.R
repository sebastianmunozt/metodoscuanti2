...
# Las hopotesis nula es de prueba, se inicia como una verdad para recolectar datos, se utiliza para verificar datos y ver si se necesita rechazar
# a favor la hipotesis alternativa. En cambio, la hipotesis alternativa es una afirmación que quiere probar o demostrar los datos recorlectados,
# hay diferencias y variables que pueden relacionarse para un estudio.

### La verdad no queda clara la diferencia entre ambas hipótesis, pues se menciona que la hipótesis nula es una verdad para recolectar datos, 
### y la hipótesis alternativa es una afirmación que quiere probar los datos recolectados. Desde esto se entiende que ambas afirmaciones se relacionan con 
### la recolección de los datos, y en realidad esto no sería correcto, pues las afirmaciones buscan probar si las variables o grupos evaluados se relacionan o no.
### No se entiende a qué se refiere con probar los datos recolectados. En este caso no se prueban los datos, sino que se evalúan variables.
...
# La estadistica descriptiva, tal como lo dice su nombre, se encarga de describir y resumir datos de una manera descriptiva,
# con herramientas para graficar y medidas de dispersión, siendo su enfoque en organizar y resumir los datos a interpretar.
# Mientras que, la estadistica inferencial se centra en generalizar e inferir información de una población grande, esto permite
# hacer evaluaciones y predicciones, pero limitada por la muestra de datos. 

### Bien :) da cuenta de la diferencia entre ambas, incluyendo aspectos como los instrumentos utilizados en cada una, los tamaños muestrales, y el enfoque de cada una. 
...
# las variables numericas correlacionales se centran en observar cuan relacionadas se encuentran algunas varaiables,
# si hablar sobre dependientes, sino que tanto pueden relacionarse.
...
base <- base %>% dplyr::rename (hora_estudio_semanal = ea_01_horas_estudio_semana,
                                carga_academica = descripcion_carga_academica,
                                satisfraccion_academica = satisfaccion_rendimiento_academico,
                                nivel_estres = ea_06_nivel_estres_ultimo_semestre)
...
valeria_carvajal <-  freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
  tb()

### Súper bien todo :) Felicitaciones!
...
base <- base %>% 
  mutate(to_01=case_when(to_01 %in% c("Bastante tiempo.", "Suficiente tiempo.")~"Tiempo disponible",
...
ggplot(valeria_carvajal, aes(x = pct, y = fct_reorder(respuestas, pct), fill=respuestas)) +
  geom_col() +

...
# 1. Género####
baseSBgenero <- read.xlsx (xlsxFile = "bases/PL/PL SM.xlsx", sheet = "genero", startRow = 2)
...
ggsave("Outpus/PL/01.1.baseSBgeneroG.png", baseSBgeneroG,scale = 1.2)
...
baseSBgeneroT <- baseSBgenero %>% 
  ...
  cols_label(Porcentaje.Area ="% Área", 
             Area ="Área", 
             Pl = "Peñalolen", 
             Porcentaje.Pl = "% Peñalolen", 
             Macul= "Macul", 
             Porcentaje.Macul = "% Macul",
             La.Florida = "La Florida", 
             Porcentaje.La.Florida = "% La Florida") %>% 
  cols_align(align = "center",columns = everything()) %>% 
  fmt_number(
    columns = c(Porcentaje.Area, Porcentaje.Pl, Porcentaje.Macul, Porcentaje.La.Florida),
    decimals = 1,
    use_seps = FALSE,
    dec_mark = ",") %>% 
  fmt_number(
    columns = c(Area, Pl, Macul, La.Florida),
    decimals = 0,
    dec_mark = ",", 
    sep_mark = ".") %>%
  grand_summary_rows (fns = list (TOTAL = "sum"), 
                      columns = c(Area, Porcentaje.Area, Pl, Porcentaje.Pl, Macul, Porcentaje.Macul, La.Florida, Porcentaje.La.Florida), 
...
#2. Edad ####
baseSBedad <- read.xlsx (xlsxFile = "bases/PL/PL SM.xlsx", sheet = "edad", startRow = 2, rows =2:22)
...
baseSBedad2 <- read.xlsx (xlsxFile = "bases/PL/PL SM.xlsx", sheet = "edad", startRow = 2, rows =27:31)
...
ggsave("Outpus/PL/02.2 baseSBedad2g.png", baseSBedad2g,scale = 1.2)

#3. Tipo Población####
baseSBorig <- read.xlsx (xlsxFile = "bases/PL/PL SM.xlsx", sheet = "tipo1", startRow = 2)
...
baseSBorigG <-ggplot (baseSBorig, aes (x=reorder (Categoría, Porcentaje.Area), y=Porcentaje.Area, fill=Categoría)) +
  ...
  theme (plot.title = element_text(hjust = 0.5),
         ...
         text=element_text(family="Calibri"))
#guardo
ggsave("Outpus/PL/06.01 baseSBorigG.png", baseSBorigG ,scale = 1.2)


# 4. Escolaridad#### 
baseSBescolaridad <- read.xlsx (xlsxFile = "bases/PL/PL SM.xlsx", sheet = "escolaridad", startRow = 2, rows =2:12)
...
#4.2. gráfico####
baseSBescolaridad <- read.xlsx (xlsxFile = "bases/PL/PL SM.xlsx", sheet = "escolaridad", startRow = 2, rows =17:21)
...
#guardo
ggsave("Outpus/PL/03.01 baseSBescolaridadG.png", baseSBescolaridadG ,scale = 1.2)


#5. NES ####
baseSBnes <- read.xlsx (xlsxFile = "bases/PL/PL SM.xlsx", sheet = "nes", startRow = 2, rows =22:26)
...
baseSBnesG <- ggplot (baseSBnes, aes (x=Categoría, y=Porcentaje.Area, fill=Categoría)) +
  geom_col() +
  theme_minimal_hgrid()+   
  labs (title = "% NES Área de Influencia", 
        x = "NES", y = "%", 
...
#guardo
ggsave("Outpus/PL/04.01 baseSBnesG.png", baseSBnesG ,scale = 1.2)
...
baseSBnest <- baseSBnes %>% 
  ...
  cols_align(align = "center",columns = everything()) %>% 
  fmt_number(
    columns = c(Porcentaje.Area, Porcentaje.Pl, Porcentaje.Macul, Porcentaje.La.Florida,Porcentaje.RM),
    decimals = 1,
    use_seps = FALSE,
    dec_mark = ",") %>% 
  fmt_number(
    columns = c(Area, Pl, Macul, La.Florida, RM),
    decimals = 0,
    dec_mark = ",", 
    sep_mark = ".") %>%  
  grand_summary_rows (fns = list (TOTAL = "sum"), 
                    columns = c(Area, Porcentaje.Area, Pl, Porcentaje.Pl, Macul, Porcentaje.Macul, La.Florida, Porcentaje.La.Florida, RM, Porcentaje.RM), 
                    formatter = fmt_number, decimals = 0, missing_text = "", sep_mark = ".") %>%  
  tab_footnote(
...
# 6. Tipo de Trabajo ####
baseSBtipotrabajo <- read.xlsx (xlsxFile = "bases/PL/PL SM.xlsx", sheet = "tipotrabajo", startRow = 2)
...
#guardo
  ggsave("Outpus/PL/05.01 baseSBtipotrabajoG.png", baseSBtipotrabajoG ,scale = 1.2)
